...
def get_pitch_bounded_line(
    H: np.ndarray, pitch_x: float
) -> tuple[tuple[int, int], tuple[int, int]]:
    """Bounds the line strictly between the top (Y=0) and bottom (Y=68) touchlines."""
    H_inv = np.linalg.inv(H)
    pts_pitch = np.array(
        [[[pitch_x, 0.0], [pitch_x, PITCH_WIDTH_Y]]], dtype=np.float32
    )
    pts_img = cv2.perspectiveTransform(pts_pitch, H_inv)[0]
    return tuple(map(int, pts_img[0])), tuple(map(int, pts_img[1]))


def snap_to_defender_edge(
    frame: np.ndarray, click_pt: tuple[float, float], goal_side: str
) -> tuple[float, float]:
    ...
    if len(x_coords) == 0:
        return click_pt
    idx = np.argmin(x_coords) if goal_side == "left" else np.argmax(x_coords)
...
def draw_offside(
    ...
    if defender:
        ...
        # Strictly bound the offside line to the calibrated pitch width.
        pt1, pt2 = get_pitch_bounded_line(H, pitch_def_x)
        cv2.line(frame, pt1, pt2, (255, 50, 50), 2, cv2.LINE_AA)
        cv2.drawMarker(
            frame,
            (int(ai_defender[0]), int(ai_defender[1])),
            (255, 50, 50),
            cv2.MARKER_CROSS,
            10,
            2,
